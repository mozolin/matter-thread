...
#define MOTION_DETECTION_THRESHOLD_CM   5     // Изменение на 5 см = движение
...
{
    ESP_LOGI(TAG_MULTI_SENSOR, "Ultrasonic fast task started");
    
    // Находим ультразвуковой датчик
    int ultrasonic_idx = -1;
    for (int i = 0; i < configured_sensors; i++) {
        if (sensors[i].config.type == SENSOR_TYPE_ULTRASONIC) {
            ultrasonic_idx = i;
            break;
        }
    }
    ...
    if (ultrasonic_idx == -1) {
        ESP_LOGE(TAG_MULTI_SENSOR, "No ultrasonic sensor found for fast task");
        vTaskDelete(NULL);
        return;
    }
    
    uint16_t endpoint_id = sensor_mapping_list[ultrasonic_idx].endpoint_id;
    
    // Настройки для быстрого реагирования
    const uint32_t SAMPLING_INTERVAL_US = 50000; // 50ms между измерениями
    const uint32_t MOTION_THRESHOLD_CM = 8;      // Порог детектирования
    const uint32_t MOTION_COOLDOWN_MS = 200;     // 200ms антидребезг
    ...
    while (true) {
        ...
        if (current_distance >= 2 && current_distance <= 400) {
            // 1. Всегда обновляем расстояние (Pressure)
            uint16_t pressure_value = current_distance * 10;
            ...
            if (err == ESP_OK && abs((int)current_distance - (int)last_distance) > 2) {
                ESP_LOGD(TAG_MULTI_SENSOR, "FAST: Distance=%lu cm", current_distance);
            }
            
            // 2. Детектирование быстрого движения
            uint64_t current_time = esp_timer_get_time();
            uint32_t distance_change = abs((int)current_distance - (int)last_distance);
            
            if (distance_change > MOTION_THRESHOLD_CM) {
                stable_counter = 0;
                
                // Проверяем антидребезг
                if ((current_time - last_motion_time) > (MOTION_COOLDOWN_MS * 1000)) {
                    // ДЕТЕКТИРОВАНИЕ ДВИЖЕНИЯ
                    if (!motion_detected) {
                        motion_detected = true;
                        
                        // Обновляем Occupancy
                        esp_matter_attr_val_t occupancy_val = esp_matter_uint8(OCCUPANCY_OCCUPIED);
                        ...
                        ESP_LOGI(TAG_MULTI_SENSOR, "FAST: Motion detected! Δ=%lu cm", distance_change);
                    }
                }
            } else {
                stable_counter++;
                
                // Если расстояние стабильно 10 циклов подряд - сбрасываем motion
                if (motion_detected && stable_counter > 10) {
                    motion_detected = false;
                    
                    esp_matter_attr_val_t occupancy_val = esp_matter_uint8(OCCUPANCY_UNOCCUPIED);
                    esp_matter::attribute::update(
                        endpoint_id,
                        OccupancySensing::Id,
                        OccupancySensing::Attributes::Occupancy::Id,
                    ...
                    ESP_LOGD(TAG_MULTI_SENSOR, "FAST: Motion cleared");
                }
            }
        ...
        // Рассчитываем время до следующего измерения
        uint64_t cycle_time = esp_timer_get_time() - cycle_start;
        if (cycle_time < SAMPLING_INTERVAL_US) {
            // Ждем оставшееся время МИКРОСЕКУНДАХ
            uint32_t delay_us = SAMPLING_INTERVAL_US - cycle_time;
            if (delay_us > 1000) {
                // Для задержек > 1ms используем vTaskDelay
                vTaskDelay(pdMS_TO_TICKS(delay_us / 1000));
            } else {
                // Малые задержки - busy wait
...
// В функции инициализации создаем быструю задачу
void app_driver_start_fast_ultrasonic_task(void)
{
    // Проверяем, есть ли ультразвуковой датчик
    ...
    if (!has_ultrasonic) {
        ESP_LOGI(TAG_MULTI_SENSOR, "No ultrasonic sensor, skipping fast task");
        return;
    }
    
    // ИСПОЛЬЗУЕМ CONFIG_ULTRASONIC_FAST_TASK_PRIORITY
    BaseType_t task_priority = CONFIG_ULTRASONIC_FAST_TASK_PRIORITY;
    
    // Создаем задачу с настроенным приоритетом
    BaseType_t result = xTaskCreate(ultrasonic_fast_task, "ultrasonic_fast", 4096, NULL, task_priority, NULL);
    
    if (result == pdPASS) {
        ESP_LOGI(TAG_MULTI_SENSOR, 
                "Fast ultrasonic task created with priority %d", 
                task_priority);
    } else {
        ESP_LOGE(TAG_MULTI_SENSOR, 
                "Failed to create fast ultrasonic task (priority %d)", 
                task_priority);
    }
}
...
esp_err_t app_driver_sensor_reinit(uint8_t sensor_idx)
{
    ...
    // Сначала деинициализируем если нужно
    switch (cfg->type) {
        case SENSOR_TYPE_MICROWAVE:
            break;
...
static esp_err_t app_driver_sensor_soft_reset(uint8_t sensor_idx)
{
    ...
    switch (sensor->config.type) {
        case SENSOR_TYPE_PIR:
            // Для PIR просто переинициализируем
        ...
        case SENSOR_TYPE_ULTRASONIC:
            // Для HC-SR04 используем специальную функцию сброса
            err = hcsr04_init(&ultrasonic_sensor, 
                             sensor->config.trigger_pin, 
                             sensor->config.echo_pin);
            // Дополнительный сброс
            if (err == ESP_OK) {
                // Даем датчику время на инициализацию
                vTaskDelay(pdMS_TO_TICKS(100));
                // Отправляем несколько тестовых импульсов
                for (int i = 0; i < 3; i++) {
    ...
    if (err == ESP_OK) {
        ...
        sensor_reset_tracker[sensor_idx].last_reset_time = esp_timer_get_time() / 1000; // в мс
...
void sensor_polling_task(void *pvParameters)
{
    ...
    // === ИНИЦИАЛИЗАЦИЯ ТРЕКЕРОВ ПЕРЕЗАГРУЗКИ ===
    for (int i = 0; i < configured_sensors; i++) {
    ...
    // Счетчик циклов для периодических операций
    uint32_t cycle_counter = 0;
    const uint32_t RESET_CHECK_INTERVAL = 100; // Проверяем сброс каждые 100 циклов
    const uint32_t STATS_LOG_INTERVAL = 5000;  // Логируем статистику каждые 5000 циклов
    ...
    while (true) {
        ...
        // === ПРОВЕРКА ПЕРИОДИЧЕСКОЙ ПЕРЕЗАГРУЗКИ ===
        if (cycle_counter % RESET_CHECK_INTERVAL == 0) {
            uint64_t current_time_ms = esp_timer_get_time() / 1000;
            
            for (int i = 0; i < configured_sensors; i++) {
                // Пропускаем, если уже в процессе сброса
                if (sensor_reset_tracker[i].reset_in_progress) {
                    continue;
                }
                
                // Проверяем, прошло ли достаточно времени с последнего сброса
        ...
        for (int i = 0; i < configured_sensors; i++) {
            // Пропускаем датчики в процессе сброса
            if (sensor_reset_tracker[i].reset_in_progress) {
            ...
            switch (sensor->config.type) {
                case SENSOR_TYPE_PIR: {
                    ...
                    if (current_state != sensor->last_state) {
                        // Минимальный интервал между изменениями - 2 секунды
                ...
                case SENSOR_TYPE_ULTRASONIC: {
                    ...
                    // новый код: улучшенная логика детектирования движения
                    bool motion_detected = false;
                    ...
                    // Проверяем изменение расстояния для детектирования движения
                    if (last_distance_cm > 0) {
                        int32_t distance_change = abs((int32_t)current_distance - (int32_t)last_distance_cm);
                        
                        // Детектируем движение если:
                        // 1. Изменение больше порога (5 см) И
                        // 2. Объект находится в пределах детектируемого диапазона (до 400 см)
                        if (distance_change >= MOTION_DETECTION_THRESHOLD_CM && 
                            current_distance <= MIN_DISTANCE_FOR_MOTION_CM) {
                            motion_detected = true;
                            //ESP_LOGW(TAG_MULTI_SENSOR, "~~~ HC-SR04: Motion detected! Change: %lu cm", distance_change);
                        }
                        
                        //ESP_LOGW(TAG_MULTI_SENSOR, "~~~ HC-SR04: Distance changed by %lu cm (%lu -> %lu)", distance_change, (int32_t)last_distance_cm, (int32_t)current_distance);
                        
                        // Обновляем давление при значительном изменении (>1 см) или каждые 2 секунды
                        ...
                        if (update_err == ESP_OK) {
                            ...
                            if (should_update_occupancy) {
                                ...
                                if (occupancy_err == ESP_OK) {
                                    ESP_LOGW(TAG_MULTI_SENSOR, "~~~ HC-SR04: Motion %s (%lu cm -> %lu cm: change %lu cm)", 
...
esp_err_t app_driver_reset_sensor_by_type(sensor_type_t type)
{
    ...
    for (int i = 0; i < configured_sensors; i++) {
        if (sensors[i].config.type == type) {
            ...
            vTaskDelay(pdMS_TO_TICKS(100)); // Пауза между сбросами
        }
    }
...
// Функция для сброса всех датчиков
esp_err_t app_driver_reset_all_sensors(void)
{
    ...
    for (int i = 0; i < configured_sensors; i++) {
        ...
        vTaskDelay(pdMS_TO_TICKS(200)); // Пауза между сбросами
    }
...
void app_driver_log_sensor_statistics(void)
{
    ...
    for (int i = 0; i < configured_sensors; i++) {
        ...
        switch (sensor->config.type) {
            case SENSOR_TYPE_MAX: type_str = "Max"; break;
