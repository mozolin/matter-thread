...
static esp_err_t create_plug(gpio_plug* plug, node_t* node)
{
    ...
    if (!node) {
        ESP_LOGE(TAG_H2, "Matter node cannot be NULL");
        return ESP_ERR_INVALID_ARG;
    }

    if (!plug) {
        ESP_LOGE(TAG_H2, "Plug cannot be NULL");
        return ESP_ERR_INVALID_ARG;
    }
    ...
    if ((reset_gpio != gpio_num_t::GPIO_NUM_NC) && (reset_gpio == plug->GPIO_PIN_VALUE)) {
        ESP_LOGE(TAG_H2, "Reset button already configured for gpio pin : %d", plug->GPIO_PIN_VALUE);
    ...
    for (int i = 0; i < configure_plugs; i++) {
        if (plugin_unit_list[i].plug == plug->GPIO_PIN_VALUE) {
            ESP_LOGE(TAG_H2, "Plug already configured for gpio pin : %d", plug->GPIO_PIN_VALUE);
            return ESP_ERR_INVALID_STATE;
        }
    }
    ...
    if (!endpoint) {
        ESP_LOGE(TAG_H2, "Matter endpoint creation failed");
        return ESP_FAIL;
    }
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_H2, "Failed to initialize plug");
    }
    ...
    if (configure_plugs < CONFIG_NUM_VIRTUAL_PLUGS) {
        ...
        ESP_LOGE(TAG_H2, "Maximum plugs configuration limit exceeded!!!");
        return ESP_FAIL;
    }

    uint16_t plug_endpoint_id = endpoint::get_id(endpoint);
    ESP_LOGI(TAG_H2, "Plug created with endpoint_id %d", plug_endpoint_id);
    return err;
}
