...
static esp_err_t app_driver_update_gpio_value(gpio_num_t pin, bool value)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_H2, "Failed to set GPIO level");
        return ESP_FAIL;
    } else {
        ESP_LOGI(TAG_H2, "GPIO pin : %d set to %d", pin, value);
    }
    return err;
}
...
esp_err_t app_driver_plugin_unit_init(const gpio_plug* plug)
{
    ...
    if (err != ESP_OK) {
        ESP_LOGE(TAG_H2, "Unable to set GPIO OUTPUT mode");
        return ESP_FAIL;
    }

    err = gpio_set_level(plug->GPIO_PIN_VALUE, 0);
    if (err != ESP_OK) {
        ESP_LOGI(TAG_H2, "Unable to set GPIO level");
    }
    return err;
}
    ...
    if (cluster_id == OnOff::Id) {
        if (attribute_id == OnOff::Attributes::OnOff::Id) {
            ...
            if (gpio_pin != GPIO_NUM_NC) {
                ...
                ESP_LOGE(TAG_H2, "GPIO pin mapping for endpoint_id: %d not found", endpoint_id);
                return ESP_FAIL;
            }
        }
    }
...
{
    VerifyOrReturnValue((reset_gpio), (app_driver_handle_t)NULL, ESP_LOGE(TAG_H2, "reset_gpio cannot be NULL"));
    ...
    *reset_gpio = gpio_num_t::GPIO_NUM_NC;
...
#endif
    ESP_LOGI(TAG_H2, "Initializing reset button with gpio pin %d ...", (int)*reset_gpio);

    // Make sure button's IO pin isn't assigned to a plug's IO pin
    for (int i = 0; i < configure_plugs; i++) {
        if (plugin_unit_list[i].plug == *reset_gpio) {
            ESP_LOGE(TAG_H2, "Button's gpio pin %d is already configured for plug %d", *reset_gpio, i);
            *reset_gpio = gpio_num_t::GPIO_NUM_NC;
            return (app_driver_handle_t)NULL;
        }
    }
    ...
    if (iot_button_new_gpio_device(&btn_cfg, &btn_gpio_cfg, &reset_handle) != ESP_OK) {
        ESP_LOGE(TAG_H2, "Failed to create button device");
        return NULL;
    }
#else
    ...
    if (btn_cnt >= 1) {
        ...
        ESP_LOGE(TAG_H2, "bsp_iot_button_create() didn't return a usable button count: %d", btn_cnt);
    }
#endif
