...
esp_err_t i2c_master_init(SSD1306_t * dev, int16_t sda, int16_t scl, int16_t reset)
{
	...
	ESP_LOGW(TAG_MIKE_APP, "~~~ New i2c driver is used");
	...
	if(err != ESP_OK) {
		ESP_LOGW(TAG_MIKE_APP, "~~~ i2c_new_master_bus() failed! %d (%s)", err, esp_err_to_name(err));
		return err;
	}
	...
	if(err != ESP_OK) {
		ESP_LOGW(TAG_MIKE_APP, "~~~ i2c_master_bus_add_device() failed! %d (%s)", err, esp_err_to_name(err));
		return err;
	}
...
esp_err_t i2c_device_add(SSD1306_t * dev, i2c_port_t i2c_num, int16_t reset, uint16_t i2c_address)
{
	...
	ESP_LOGW(TAG_MIKE_APP, "~~~ New i2c driver is used");
	ESP_LOGW(TAG_MIKE_APP, "~~~ Will not install i2c master driver");
#if 0
	i2c_master_bus_config_t i2c_mst_config = {
		.clk_source = I2C_CLK_SRC_DEFAULT,
		.glitch_ignore_cnt = 7,
	...
	err = i2c_master_bus_add_device(dev->_i2c_bus_handle, &dev_cfg, &i2c_dev_handle);
	if(err != ESP_OK) {
		ESP_LOGW(TAG_MIKE_APP, "~~~ i2c_master_bus_add_device() failed! %d (%s)", err, esp_err_to_name(err));
		return err;
	}
...
esp_err_t i2c_init(SSD1306_t * dev, int width, int height)
{
	...
	esp_err_t err;
	err = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, out_index, I2C_TICKS_TO_WAIT);
	if(err == ESP_OK) {
		ESP_LOGW(TAG_MIKE_APP, "~~~ OLED configured successfully");
	} else {
		ESP_LOGE(TAG_MIKE_APP, "~~~ Could not write to device [0x%02x at %d]: %d (%s)", dev->_address, dev->_i2c_num, err, esp_err_to_name(err));
		err = ESP_FAIL;
	}
	return err;
}
...
void i2c_display_image(SSD1306_t * dev, int page, int seg, const uint8_t * images, int width) {
	...
	if (out_buf == NULL) {
		ESP_LOGE(TAG_MIKE_APP, "~~~ malloc fail");
		return;
	}
	int out_index = 0;
	out_buf[out_index++] = OLED_CONTROL_BYTE_CMD_STREAM;
	...
	err = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, width + 1, I2C_TICKS_TO_WAIT);
	if (err != ESP_OK)
		ESP_LOGE(TAG_MIKE_APP, "~~~ Could not write to device [0x%02x at %d]: %d (%s)", dev->_address, dev->_i2c_num, err, esp_err_to_name(err));
	free(out_buf);
}
...
void i2c_contrast(SSD1306_t * dev, int contrast) {
	...
	esp_err_t err = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, 3, I2C_TICKS_TO_WAIT);
	if (err != ESP_OK)
		ESP_LOGE(TAG_MIKE_APP, "~~~ Could not write to device [0x%02x at %d]: %d (%s)", dev->_address, dev->_i2c_num, err, esp_err_to_name(err));
}
...
void i2c_hardware_scroll(SSD1306_t * dev, ssd1306_scroll_type_t scroll) {
	...
	esp_err_t err = i2c_master_transmit(dev->_i2c_dev_handle, out_buf, out_index, I2C_TICKS_TO_WAIT);
	if (err != ESP_OK)
		ESP_LOGE(TAG_MIKE_APP, "~~~ Could not write to device [0x%02x at %d]: %d (%s)", dev->_address, dev->_i2c_num, err, esp_err_to_name(err));
}
